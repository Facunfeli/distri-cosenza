...
const ITrash2 = () => React.createElement(Ico,{d:'M3 6h18M19 6l-1 14H6L5 6M10 11v6M14 11v6M9 6V4h6v2'})
  ...
  const go = async () => {
    ...
    try {
      if (u.trim() === 'admin' && p.trim() === 'admin123') {
        sessionStorage.setItem('dc_session', JSON.stringify({role:'admin'}))
        onLogin({role:'admin'}); return
      }
      const {data,error} = await supabase.from('clients').select('*').eq('username',u.trim()).eq('password',p.trim()).single()
      if (error || !data) { setErr('Usuario o contrasena incorrectos'); setLoading(false); return }
      sessionStorage.setItem('dc_session', JSON.stringify({role:'client',client:data}))
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:C.bg,padding:24}}>
      <div style={{width:'100%',maxWidth:360,background:C.card,borderRadius:20,padding:'36px 28px',border:'1px solid #262626'}}>
        ...
        <input style={inp} placeholder="Usuario" value={u} onChange={e=>{setU(e.target.value);setErr('')}} onKeyDown={e=>e.key==='Enter'&&go()}/>
        <input style={inp} placeholder="Contrasena" type="password" value={p} onChange={e=>{setP(e.target.value);setErr('')}} onKeyDown={e=>e.key==='Enter'&&go()}/>
...
function AdminClients({clients,setClients,products,cats}) {
  ...
  const openNew = () => {
    ...
    setPrices(blankPrices()); setSelCat(cats[0]?.id||null); setWspSent(false); setEditId(null); setView('edit')
  }
  ...
  const openEdit = (c) => {
    ...
    setPrices(p); setSelCat(cats[0]?.id||null); setWspSent(false); setEditId(c.id); setView('edit')
  }
  ...
  const save = async () => {
    ...
    try {
      if(!editId) {
        ...
        const {data,error} = await supabase.from('clients').update(entry).eq('id',editId).select().single()
  ...
  if(view==='edit') {
    ...
    return (
      <div>
        ...
        <select style={{...inp}} value={form.type||'Almacen'} onChange={e=>setForm(f=>({...f,type:e.target.value}))}>
        ...
        <input style={inp} placeholder="Usuario *" value={form.username} onChange={e=>setForm(f=>({...f,username:e.target.value.toLowerCase().replace(/\s/g,'')}))}/>
        <input style={inp} placeholder="Contrasena" value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))}/>
        <input style={inp} placeholder="WhatsApp (ej: 5491165001234)" value={form.phone||''} onChange={e=>setForm(f=>({...f,phone:e.target.value}))}/>
        {form.phone&&(
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:6}}>
              {[['Precio $','price'],['Min. doc.','min_qty'],['% desc. bulto','bulk_discount']].map(([lbl,field])=>(
                <div key={field}>
                  ...
                  <input style={{...inp,marginBottom:0,padding:'9px 10px',fontSize:13}} type="number"
        ...
        <div key={c.id} style={cardS}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            ...
            <div style={{display:'flex',gap:6,flexShrink:0,marginLeft:8}}>
              ...
              <button style={{...btnS('sm'),background:'rgba(224,92,92,0.1)',color:C.red,border:'none'}} onClick={()=>del(c.id)}><ITrash2/></button>
            </div>
          </div>
        </div>
      ))}
...
function AdminProducts({products,setProducts,cats}) {
  ...
  const save = async () => {
    ...
    const entry = {
      ...
      qty_per_unit: form.qty_per_unit||12,
      cat: form.cat||'',
    ...
    try {
      if(editing==='new') {
        ...
        const {data,error} = await supabase.from('products').update(entry).eq('id',editing).select().single()
  ...
  const addNew = () => {
    setForm({name:'',description:'',unit:'docena',qty_per_unit:12,cat:cats[0]?.id||'',photo:null,photo2:null,active:true})
    setEditing('new')
  }
  ...
  if(editing!==null) return (
    <div>
      ...
      <textarea style={{...inp,minHeight:60,resize:'vertical'}} placeholder="Descripcion breve" value={form.description||''} onChange={e=>setForm(f=>({...f,description:e.target.value}))}/>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:10}}>
        ...
        <div><p style={{fontSize:11,color:C.muted,marginBottom:4}}>Unidades/pack</p><input style={{...inp,marginBottom:0}} type="number" value={form.qty_per_unit||12} onChange={e=>setForm(f=>({...f,qty_per_unit:Number(e.target.value)}))}/></div>
      </div>
  ...
  return (
    <div>
          ...
          {items.map(p=>(
            <div key={p.id} style={{...cardS,display:'flex',gap:12,alignItems:'center',padding:12}}>
              ...
              <div style={{display:'flex',gap:6}}>
                <button style={btnS('sm')} onClick={()=>{setForm({...p});setEditing(p.id)}}>Editar</button>
      ...
      {sinCat.length>0&&<div><p style={{color:C.red,fontSize:11,marginBottom:8}}>Sin categoria</p>{sinCat.map(p=><div key={p.id} style={{...cardS,display:'flex',justifyContent:'space-between'}}><p style={{color:C.text,margin:0}}>{p.name}</p><button style={btnS('sm')} onClick={()=>{setForm({...p});setEditing(p.id)}}>Editar</button></div>)}</div>}
    </div>
  )
}
...
function Store({session,onLogout}) {
  ...
  const confirm = async () => {
    setSubmitting(true)
    await supabase.from('orders').insert([{client_id:client.id,items:cartItems,total:cartTotal}])
  ...
  if(view==='cart') return (
    <div style={{minHeight:'100vh',background:C.bg}}>
      ...
      <div style={{padding:16}}>
        ...
        <div style={{...cardS,background:'#111',display:'flex',justifyContent:'space-between',alignItems:'center',padding:'16px 14px'}}><p style={{fontWeight:700,fontSize:16,color:C.text,margin:0}}>Total</p><p style={{fontWeight:800,fontSize:24,color:C.gold,margin:0}}>{fmt(cartTotal)}</p></div>
        <div style={{height:8}}/>
    ...
    <div style={{minHeight:'100vh',background:C.bg,paddingBottom:90}}>
      ...
      <div style={{padding:'10px 12px'}}>
        {visibleProds.map(pr=>{
          ...
          const priceUnit=Math.round(Number(pdata.price)/qpu)
          const bulkPrice=pdata.bulk_discount>0?Math.round(Number(pdata.price)*(1-pdata.bulk_discount/100)):0
            ...
            <div key={pr.id} style={{background:C.card,border:'1px solid #262626',borderRadius:14,marginBottom:8,overflow:'hidden'}}>
              ...
              <div style={{padding:14}}>
                ...
                <div style={{background:'rgba(201,169,110,0.1)',border:'1px solid rgba(201,169,110,0.25)',borderRadius:8,padding:'7px 12px',marginBottom:bulkPrice?8:12,display:'flex',alignItems:'center',gap:8}}>
                  ...
                  <span style={{fontSize:14,fontWeight:700,color:C.gold}}>Minimo: {pdata.min_qty||1} {pr.unit} = {(pdata.min_qty||1)*qpu} unidades</span>
                </div>
                {bulkPrice>0&&(
                ...
                <p style={{fontSize:13,color:'#777',margin:'0 0 12px',lineHeight:1.5}}>{pr.description}</p>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                  ...
                  {qty>0&&<div style={{textAlign:'right'}}><p style={{fontWeight:800,color:C.green,fontSize:18,margin:0}}>{fmt(Number(pdata.price)*qty)}</p><p style={{fontSize:11,color:C.muted,margin:'2px 0 0'}}>{qty*qpu} unidades</p></div>}
                </div>
              </div>
            </div>
          )
        })}
        {visibleProds.length===0&&<p style={{color:C.muted,textAlign:'center',padding:'40px 0',fontSize:14}}>No hay productos en esta categoria.</p>}
      </div>
      {cartCount>0&&(
